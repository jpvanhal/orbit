...
import { verifyLocalForageContainsRecord, verifyLocalForageIsEmpty } from 'tests/test-helper';
...
module("OC - LocalForageSource", {
  setup: function() {
    ...
    stop();
    window.localforage.setDriver(window.localforage.LOCALSTORAGE).then(function() {
      start();
    });
